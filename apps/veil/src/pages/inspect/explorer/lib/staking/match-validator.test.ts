...
import { findValidatorInfo, identityKeyOf, stakeGate, totalDelegatedUm } from './match-validator';
...
/**
 * A delegation ValueView carrying its validator's exchange rate, the shape
 * `ViewService.delegationsByAddressIndex` returns.
 */
const delegationView = (tokens: number, exchangeRate: bigint): ValueView =>
  new ValueView({
    valueView: {
      case: 'knownAssetId',
      value: {
        amount: { lo: BigInt(tokens * 1e6), hi: 0n },
        metadata: new Metadata({ display: 'delegation', denomUnits: [] }),
        extendedMetadata: {
          typeUrl: ValidatorInfo.typeName,
          value: new ValidatorInfo({
            validator: new Validator({
              identityKey: new IdentityKey({ ik: new Uint8Array(Array(32).fill(0xaa)) }),
            }),
            rateData: new RateData({ validatorExchangeRate: { lo: exchangeRate, hi: 0n } }),
          }).toBinary(),
        },
      },
    },
...
describe('totalDelegatedUm', () => {
  it('converts each delegation through its own validator exchange rate', () => {
    // 1e8 == rate 1.0, 1.5e8 == rate 1.5.
    const total = totalDelegatedUm([delegationView(100, 100_000_000n)]);
    expect(total.um).toBeCloseTo(100, 6);
    expect(total.unconverted).toBe(0);

    const grown = totalDelegatedUm([delegationView(100, 150_000_000n)]);
    expect(grown.um).toBeCloseTo(150, 6);
  });

  it('sums across validators rather than adding raw token amounts', () => {
    // Raw token sum would be 200; the correct UM total is 100*1.0 + 100*2.0.
    const total = totalDelegatedUm([
      delegationView(100, 100_000_000n),
      delegationView(100, 200_000_000n),
    ]);
    expect(total.um).toBeCloseTo(300, 6);
  });

  it('counts delegations it cannot convert instead of dropping them silently', () => {
    const total = totalDelegatedUm([new ValueView({})]);
    expect(total.um).toBe(0);
    expect(total.unconverted).toBe(1);
  });

  it('is zero for no delegations', () => {
    expect(totalDelegatedUm([])).toEqual({ um: 0, unconverted: 0 });
  });
});
