...
const FAQS: FAQ[] = [
  ...
  },
  {
    ...
    a: 'Shielding means moving assets into the encrypted balance pool. The recommended path is USDC over IBC from Injective, straight to a Penumbra address; your wallet handles the deposit. Noble, the chain that issues native USDC in Cosmos, remains supported and is the source of the USDC already shielded today. ATOM, OSMO, TIA and any IBC asset shield the same way: send an IBC transfer from the originating Cosmos chain to your Penumbra address. After one block confirmation, the asset appears in your shielded balance and is invisible to chain analysis.',
  },
  {
  ...
  <>
    ...
    <section className='mx-auto flex max-w-[820px] flex-col gap-8 p-4 desktop:py-10'>
      ...
      <div className='flex flex-col gap-3'>
        {FAQS.map(({ slug, q, a }) => (
          <FaqEntry key={slug} slug={slug} q={q} a={a} />
        ))}
      </div>
